...
config_location_assets() {
    ...
    location /assets {
        ...
        '"$(gen_api_get_list_sub_filter)"'
        sub_filter '"'Aliyundrive('"' '"'AliyundriveDel('"';
        sub_filter_once off;
        sub_filter_types *;
...
config_ui() {
    ...
    if [ -z "$sign" ]; then
        ...
        NEW_CONFIG='
                ...
                sub_filter_once off;
                sub_filter_types *;
                sub_filter '"'.hope-ui-light{'"' '"'.hope-ui-light-del{'"';
                sub_filter '"'.hope-ui-dark{'"' '"'.hope-ui-dark-del{'"';
                sub_filter '"'/* 去掉右上角搜索框 */'"' '"'.hope-c-PJLV .hope-c-PJLV-ikSuVsl-css {display: none !important;}'"';
'
    fi
    ...
    in_target_block {
        while (getline > 0) {
            if (!(/Accept-Encoding/ || /sub_filter_once/ || /sub_filter_types/ || /sub_filter/)) {
                print
            }
            if (/}[[:space:]]*$/) {
                break
