    ...
    public String homeContent(boolean filter) throws Exception {
        ...
        if (initTest.isEmpty()) {
            ...
            Thread thread = new Thread(() -> {
                defaultDrive.exec("{ awk -F'#' 'NF<2{print;next} tolower($0)~/iso#/{a[$3]=(a[$3]?$1\"~~~\"a[$3]:$0);next} {b[$3]=(b[$3]?$1\"~~~\"b[$3]:$0)} END{for(i in b)print b[i];for(i in a)print a[i]}' index.combined.txt > index.tmp.txt&&mv -f index.tmp.txt index.combined.txt&&echo '~~~~~~~~~~'>>index.video.txt; } < /dev/null > /dev/null 2>&1 &"); 
            });
            thread.start();
        }
